...
import { dateLabel, Empty, ErrorNotice, Loading, Modal, sessionLabels, unavailable } from './ui/common';
...
function Workspace({ store, account, demo }: { store: DesktopStore; account: Account; demo: boolean }) {
  ...
  return <div className="app-shell">
    ...
    {dialog && <ActionDialog key={JSON.stringify(dialog)} dialog={dialog} account={account} api={api} busy={state.busy} error={state.error} run={(work, refresh) => store.action(work, refresh)} onClose={closeDialog} onLogout={() => store.logout()} />}
