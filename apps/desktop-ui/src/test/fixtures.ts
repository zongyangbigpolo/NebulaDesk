...
export const account: Account = { id: 'owner', display_name: '测试用户', email: 'owner@example.test', tenant: 'test-space', manager_url: 'https://manager.example.test', role: 'USER', workspace: { id: 'workspace', slug: 'test-space', name: '测试空间', kind: 'ORGANIZATION' } };
...
export function fakeApi(overrides: Handlers = {}): DesktopApi & { calls: Request[] } {
  const handlers: Handlers = { connection_settings: () => ({ manager_url: null }), account: () => account, resources: () => [resource], machines: () => [machine], local_host: () => host, sessions: () => [], transfers: () => [], resource: () => resource, machine_resources: () => [], grants: () => [], logout: () => null, ...overrides };
  const calls: Request[] = [];
  return {
    calls,
