...
import { dateLabel, Empty, ResourceArt, sessionLabels, unavailable } from './common';
  ...
  return <>
    ...
    {!visible.length ? <Empty title={resources.length ? '没有匹配的资源' : '还没有可连接的资源'}>{resources.length ? '试试其他关键词或资源类型。' : '添加自己的设备，或请所有者通过你的账号邮箱授予访问权限。'}</Empty> : <div className="resource-grid">{cards.map(resource => {
      ...
      return <article className="resource-card" key={resource.id}>
        <button className="art-button" aria-label={`查看 ${resource.name} 详情`} onClick={() => onDetails(resource)}><ResourceArt resource={resource} />{session && <span className="art-badge"><span className={`dot ${session.state === 'connected' ? 'green' : ''}`} />{sessionLabels[session.state]}</span>}</button>
