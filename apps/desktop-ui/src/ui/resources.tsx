...
export function Resources({ resources, sessions, query, filter, sort, busy, onFilter, onSort, onAdd, onDetails, onConnect, onFocus, onDisconnect }: {
  ...
  onAdd: () => void; onDetails: (resource: Resource) => void; onConnect: (resource: Resource) => void; onFocus: (session: Session) => void; onDisconnect: (session: Session) => void;
}) {
  ...
  return <>
    ...
    <p className="footnote">连接会在独立的原生窗口中打开。关闭应用窗口会请求远端正常关闭；仅断开连接请使用“当前连接”。</p>
  </>;
}
