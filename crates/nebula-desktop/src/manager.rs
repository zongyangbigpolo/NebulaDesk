...
pub fn validate_url(input: &str, allow_http: bool) -> Result<Url> {
    ...
    let local = match url.host() {
        Some(reqwest::UrlHost::Ipv4(ip)) => ip.is_loopback() || ip.is_private(),
        Some(reqwest::UrlHost::Ipv6(ip)) => ip.is_loopback() || ip.is_unique_local(),
        Some(reqwest::UrlHost::Domain(host)) => host == "localhost" || host.ends_with(".localhost"),
        None => false,
    };
...
impl Manager {
    ...
    async fn raw(
        ...
        let mut response = request.send().await.map_err(DesktopError::network)?;
        if !response.status().is_success() {
            return Err(match response.status() {
                StatusCode::UNAUTHORIZED => DesktopError::new("unauthorized", "Sign in again to continue."),
                StatusCode::FORBIDDEN => DesktopError::new("forbidden", "Your account is not permitted to perform this action."),
                StatusCode::NOT_FOUND | StatusCode::METHOD_NOT_ALLOWED => DesktopError::new("unavailable", "The resource or manager capability is unavailable."),
                StatusCode::CONFLICT => DesktopError::new("conflict", "The resource is unavailable or this operation conflicts with its current state."),
...
impl Account {
    pub async fn request<T: DeserializeOwned>(
        &self, method: Method, path: &str, body: Option<&serde_json::Value>,
    ) -> Result<T> {
        // Serialize refresh rotation with authenticated requests: never race two refreshes.
        let mut credentials = self.credentials.lock().await;
        let first = self.manager.request(method.clone(), path, Some(&credentials.access), body).await;
        match first {
            Err(ref error) if error.code == "unauthorized" => {
                let pair: TokenPair = self.manager.request(
                    Method::POST, "v1/auth/refresh", None,
                ...
                self.manager.request(method, path, Some(&credentials.access), body).await
            }
            result => result,
        }
    }
    ...
    pub async fn logout(&self) -> Result<()> {
        let mut credentials = self.credentials.lock().await;
        let result = self.manager.request::<serde_json::Value>(
...
mod tests {
    ...
    fn manager_url_safety() {
        for input in ["http://example.com", "https://user:secret@example.com", "https://example.com/?token=secret", "file:///tmp/a"] {
        ...
        assert_eq!(validate_url("https://manager.example/proxy", false).unwrap().as_str(), "https://manager.example/proxy/");
    }
