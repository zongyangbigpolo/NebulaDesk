...
授权按会话边界生效（2026-09-09 产品决策）：Manager 在新会话申请时检查当前授权，
已建立的会话沿用签发时的权限，不因资源授权变更而被主动踢出。Direct/Relay 切换属于
同一逻辑会话；结束后再次连接需重新申请，撤权后不再批准新的会话。
已签发但尚未使用的票据仍受原有短有效期和单次兑换约束，不增加即时吊销机制。
立即终止被控端连接仍可通过停止 Agent 共享服务完成；这不是修改资源授权的副作用。
