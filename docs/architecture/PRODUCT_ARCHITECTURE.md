...
它把明确的用户操作交给 Rust，渲染返回的业务数据。登录失败时密码仅保留在当前表单
内存中供重试，成功、切换服务器或离开登录表单时清空；注册与邀请码提交仍清空敏感字段。
...
动态文档回归通过作用域内的 Cmd+N 创建第三个真实窗口，确认新 Surface 独立解码，
再仅关闭新窗口，原来的两个窗口仍持续出图且身份、几何不变；预期窗口数严格为
2→3→2，不放宽系统辅助窗口过滤。这是夹具原生快捷键，不代表远端全局菜单已支持。
每次运行通过新启动进程的 PID 匹配状态文件，不使用可能被
其他夹具覆盖的 latest 文件。失败后只清理本次测试应用，不退出其他已打开的用户应用。
原生 Client 的窗口呈现与两机互通仍需另外验收。

另用独立测试身份运行实际 release 包内的原生 Client，通过本地 Client 窗口执行最小化
和恢复，未对源窗口执行 AX 操作：远端最小化约 1.93 秒、恢复状态约 1.22 秒，最迟在
3.97 秒的截图中确认恢复后的新画面，窗口 ID 保持不变。另一文档在宿主 `AXWindows`
查询超时期间短暂黑屏后恢复，因此严格的“另一窗口全程不中断”尚未通过；不能靠沿用
旧画面或放宽窗口归属校验掩盖此限制。这仍是同机回环，不是两机验收。
...
上述协议回归使用 debug 构建，不能直接当作 release 包的性能结论。另一轮 release GUI
日志中，两个窗口各自从开始采集到采集启动完成约 0.493/0.475 秒，恢复窗口从开始采集
到首个编码帧约 0.268 秒；这些仅是采集子阶段，不是连接或用户操作的端到端耗时。

### 11.4 2026-09-14 的定向修复与剩余限制

`AXWindows` 发现查询超时时，已在采集的普通文档窗口可通过独立的新鲜 CG/AX 检查继续
采集；必须重新确认进程身份、可见性、几何、父子关系及内容归属。已有暂停的 epoch、
最小化窗口、弹窗与附带 sheet 的文档不能借此恢复；输入仍需要完整的新鲜窗口关系。
检查共享 250ms 软预算，AX 请求使用剩余时间，超时后的成功结果也不采纳；
不可取消的 CG/LaunchServices 调用不构成严格的墙钟耗时保证。

定向原生夹具在 20 次注入的发现超时中收到 33 个时间戳递增的编码帧，并确认隐藏、
最小化、附带 sheet 和已经关闭的窗口被排除。这不是两机或 Client GUI 验收。
另一次完整的本机原生协议回归在 debug 模式下以 75.97 秒通过，但最小化阶段仍出现
两窗同时短暂不可用，之后恢复；因此“另一窗口全程不中断”仍未通过，不能称黑屏彻底消除。

启动期间另出现过进程仍存活、LaunchServices 查询却不可用的失败。现在只在初次启动时
...
不超过两秒。该回归在 9 月 14 日的 release 运行中仍失败。诊断显示另一窗口开始验证时还剩 248ms，
但 `AXHidden` 请求约 224ms 后返回 `-25204`；改变查询顺序的实验仍在 `AXRole`
请求上约 211ms 后超时，实验已撤销。该次失败不是另一窗口抢占了预算，而是新鲜 AX
事实暂不可得；验证失败时仍必须暂停，不能通过省略归属检查或放宽验收来声称连续性通过。

### 11.5 2026-09-15 的最小化过渡诊断

新增显式启用的原生夹具诊断，同时比较保留的 AX 引用、新线程取得的引用和独立进程。
一次最小化 setter 约 0.19ms 返回成功，随后的 `AXWindows` 也成功；三个观察者稍后的
新鲜 `AXHidden` 请求却均超时。同期主线程普通模式与公共模式心跳最大间隔约
636/614ms，后台心跳仍约 100ms，最小化完成回调处于 `dockmsg-mode`。这次证据排除了
“前一次发现超时污染引用”的解释，指向 Dock 过渡期间应用暂未服务相关 AX 请求；
并不证明其他应用、其他故障都具有同一原因。

现在在最小化和恢复操作前额外获取一次新鲜快照，再执行原来的作用域与身份校验。
正常轮询间隔仍为 200ms；额外检查的下次期限从检查开始计算，不能从原生操作返回或
慢采集启动结束后重新计时。仅带捕获验证期限的属性读取，在 `-25204` 后允许利用剩余
预算再发起一次新请求。单次 RPC 上限仍为 200ms、整轮独立验证软预算仍为 250ms；
非超时错误、输入验证以及没有期限的读取不重试，晚到结果仍拒绝。失败结果不是权限
凭据，所有进程、可见性、几何、关系和内容归属检查仍须成功。

未修改断言的本机 release 严格回归首次通过，兄弟窗口 generation 保持 1，
无 Unavailable/Remove，收到 139 个解码帧，最大间隔 73.396ms；其中 `AXHidden`
新请求在累计 218ms 后成功，完整独立验证约 227ms。随后限定的三次复跑均通过，
分别收到 150/152/151 帧，最大间隔 43.755/39.412/41.981ms。只有首次运行和第一次
复跑记录到成功的属性重试，不能据此把全部改善归因于重试，也不构成跨应用连续性保证。

首次负例复核未全部完成：错误几何、隐藏应用检查通过，但采集启动后的 live 身份
查询返回 LaunchServices lookup unavailable，测试在最小化操作前失败，未执行后续
最小化、sheet 和关闭旧元素检查。该进程此前已完成启动确认，不能按启动等待问题处理；
现有身份检查仍拒绝访问，不用旧对象、进程路径或延长 live 重试来掩盖失败。

后续只在测试中显式启用进程身份观测，记录隐藏、恢复、采集启动和最小化前后的状态。
该次完整负例通过，包含最小化、sheet、关闭旧元素检查和 32 个时间戳推进的编码帧，
但未复现前次 live lookup 失败；插入观测会改变时序，不能把此次通过称为故障已修复。
公开 `NSWorkspace.runningApplications` 枚举在该次运行中一直遗漏活的目标进程，
正常退出后保留的 `NSRunningApplication` 对象仍报告原身份和未退出，主线程观测回调
也未在 100ms 内执行。这些来源不能充当授权兜底。相关非 ignored 单测 23 项通过。
原生诊断、严格回归和负例日志保留在 `target/native-ax-transaction-20260915/`，
其中 `verification-970352b.json` 和 `identity-diagnostic-summary.json` 记录精确命令、
逐次结果及观测局限。

上述均为本机原生夹具与协议路径，不是包内 Client GUI 或双 Mac APP 验收。
远端已独立暂存此前的 `4eabe44` 候选包，未激活；该包不包含本节后续修正。
真实双机仍需正常 GUI 注册并开启共享，不以 SSH 启动 helper 替代 GUI 授权流程。
