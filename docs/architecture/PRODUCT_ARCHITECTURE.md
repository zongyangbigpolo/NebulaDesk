...
关闭，以及真实 NSScrollView 滚动和 NSSlider 拖动；不会自动授予系统权限。
滚轮回归发现 Apple ARM64 下原 FFI 将固定的首轴参数错误声明为可变参数，已改用公开、
非可变参数的 `CGEventCreateScrollWheelEvent2`，桌面与 APP 共用此修复，并检查双轴正负值。
本机完整回归观察到滚动偏移从 0 到 60、滑块从 0.25 到 1.0；拖动必须同时满足由原生
控件几何计算的终点值容差，并在正确文档终点收到 MouseDrag 和 MouseUp 后才继续缩放。
这些输入通过测试客户端的加密网络路径投递，不是两机 GUI 输入验收。
另已覆盖 `Minimize` 后通过 `Focus` 恢复：同时检查真实 NSWindow 最小化状态与协议元数据，
保持原 Surface ID，另一文档在最小化期间继续解码；恢复必须使用新 generation 的画面，
不能把暂停前的缓存算作恢复成功。正常关闭与最终 Bye 仍须通过。
每次运行通过新启动进程的 PID 匹配状态文件，不使用可能被
其他夹具覆盖的 latest 文件。失败后只清理本次测试应用，不退出其他已打开的用户应用。
原生 Client 的窗口呈现与两机互通仍需另外验收。

### 11.3 原生交互延迟记录

本机同一真实回归增加了发送与夹具观察计时。移除输入路径中不必要的整棵 AX 树遍历、
使用新鲜 CG 信息和精确 AX 身份校验后，首次 A/B/C 批次从最后一次发送到全部观察到的
时间由约 4.891 秒降至 1.727 秒；Reset 点击对应值从约 1.404 秒降至 0.052 秒。
这些是特定本机运行的观测，不是跨机性能承诺，也不是每次按键都耗时 1.727 秒。

初始批次仍受单次原生操作阻塞：A 按下的 CG 投递约 766ms，随后 A 抬起的 CG 查询约
722ms；后续 B/C 原生投递未触发 100ms 慢操作日志，但曾排在前两者之后等待。
不能据此断言原因一定是冷初始化或声称稳态端到端延迟低于 100ms。
从加密 ControlHello 到首窗/第二窗解码约 15.254/18.987 秒，原生采集启动仍偏慢，
不将“使用 QUIC”本身当作极低启动或交互延迟的证明。
本机最小化回归中，最小化状态确认约 1.124 秒，Focus 到恢复窗口及新画面确认约
8.952 秒（包含两窗恢复等待）；这不是瞬时恢复体验，仍需继续优化原生采集启动路径。
